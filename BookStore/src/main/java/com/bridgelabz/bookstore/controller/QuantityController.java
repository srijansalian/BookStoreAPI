 ...
 * 
 * 
 * @PostMapping("/addquantitytocart{cartId}") public
 * ResponseEntity<BookResponse> addQuantity(@PathVariable( value = "cartId")
 * long cartId,@RequestHeader long id) {
 * Quantityservice.addtocartquantity(cartId,id);
 * 
 * return ResponseEntity.status(HttpStatus.CREATED).body(new
 * BookResponse("The Quantity details are added to cart",cartId)); }
 * 
 * 
 * }
 * 
